...
func play_card_zoom_animation(card_idx: int) -> void:
	...
	# 將縮放中心點設在卡片正中心
	card_widget.pivot_offset = card_widget.size / 2.0
	...
	# 第一次縮放
	zoom_tween.tween_property(card_widget, "scale", target_scale, duration_up).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	zoom_tween.tween_property(card_widget, "scale", normal_scale, duration_down).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
...
# 播放敵人反擊老奶奶的表演
func play_enemy_attack(item_name: String, damage: int, from_player_hp: int, to_player_hp: int, max_player_hp: int) -> void:
	var grandma_sprite = combat_ui.get_node("PlayerSide/GrandmaSprite")
	var enemy_sprite = combat_ui.enemy_sprite
	...
	# 可以載入特定道具 icon 或預設彈珠
	var proj_tween = proj.setup(start_pos, target_pos, null)
...
# 輔助：生成傷害數字飄字
func spawn_damage_popup(pos: Vector2, amount: int, is_heal: bool, is_player: bool) -> void:
	var popup = damage_popup_scene.instantiate()
	combat_ui.add_child(popup)
	popup.global_position = pos - Vector2(50, 20) # 微調置中
	popup.setup(amount, is_heal, is_player)
