...
## 讀取 skills.csv，回傳 { skill_id(int): { type, display, color } }
static func _load_skill_table() -> Dictionary:
	...
	while not file.eof_reached():
		...
		table[int(cols[0])] = {
			...
			"color":   Color.html(cols[3].strip_edges())
		}
...
static func load_all() -> Array[CardResource]:
	...
	while not file.eof_reached():
		...
		# 說明：Excel 開啟 CSV 會將「00011000」等前導零字串當數字直接展示為「11000」
		...
		var default_info := { "type": "damage", "display": "攻擊", "color": Color(0.6, 0.6, 0.6) }
		...
		card.skill_a_color   = skill_a_info["color"]
		card.skill_b_display = skill_b_info["display"]
		card.skill_b_color   = skill_b_info["color"]
		
		# 依技能 B 的 skill_type 設定 bool flag
		var skill_b_type: String = skill_b_info["type"]
		card.skill_b_is_heal        = (skill_b_type == "heal")
		card.skill_b_is_delay_cd    = (skill_b_type == "delay_cd")
		card.skill_b_is_damage_buff = (skill_b_type == "damage_buff")
		...
		cards.append(card)
		print("CardDataLoader: 載入卡牌 [%s]" % card.animal_name)
