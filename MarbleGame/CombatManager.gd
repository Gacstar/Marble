...
signal combat_updated(player_hp, player_max_hp, monsters_data, target_idx)
signal monster_attacked(monster_name, damage)
signal monster_defeated(monster_name)
...
signal monster_selection_changed(idx)

var player_max_hp = 100
var player_hp = 100

var monsters: Array[Dictionary] = []
var target_monster_idx: int = 0
...
func reset_combat():
	...
	monsters = [
		{
			"name": "Heavy Monster A",
			"icon": load("res://assets/monster_enemy.jpg"),
			"hp": 500,
			"max_hp": 500,
			"cd": 4,
			"cd_default": 4,
...
func select_monster(idx: int):
	if idx >= 0 and idx < monsters.size():
		if monsters[idx].hp > 0:
			target_monster_idx = idx
			monster_selection_changed.emit(idx)
			emit_combat_signal()
...
func trigger_skill_from_slot(slot_index: int):
	...
	var target = monsters[target_monster_idx]
	...
	if is_skill_b and card.skill_b_is_heal:
		...
		if target.hp <= 0:
			target.hp = 0
			monster_defeated.emit(target.name)
...
func _auto_select_next_target():
	for i in range(monsters.size()):
		if monsters[i].hp > 0:
			...
			monster_selection_changed.emit(i)
			return
...
func monster_turn_tick():
	for m in monsters:
		...
		if m.cd <= 0:
			...
			m.cd = m.cd_default
			monster_attacked.emit(m.name, damage)

func emit_combat_signal():
	combat_updated.emit(player_hp, player_max_hp, monsters, target_monster_idx, next_damage_multiplier)
